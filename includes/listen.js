  ...
  return async (event) => {
    ...
    if (
      ...
      try {
        rentals = JSON.parse(
          require("fs").readFileSync(
            process.cwd() + "/modules/commands/cache/data/approve.json",
          ),
        );
      } catch {
        rentals = [];
      ...
      let find_rental = rentals.find(($) => $.t_id == event.threadID);
      if (!find_rental && event.body.trim() !== `${prefix}callad`) {
        if (event && event.threadID) {
          return api.shareContact(
            `[  Bot Rental Notice  ]\n─────────────────\n❌ Your group has not activated bot usage, please use "${prefix}callad" command to contact Admin.\n─────────────────\n🌐 Facebook:`,
            global.config.ADMINBOT[0],
            event.threadID,
          );
        } else {
          console.error("Error when shareContact");
          return;
        }
      }
      ...
      if (
        find_rental &&
        new Date(form_mm_dd_yyyy(find_rental.time_end)).getTime() <=
          Date.now() + 25200000 &&
        event.body.trim() !== `${prefix}callad`
      ) {
        if (event && event.threadID) {
          return api.shareContact(
            `[  Bot Rental Notice  ]\n─────────────────\n❌ Your group's bot usage time has expired, please use "${prefix}callad" command to contact Admin for renewal.\n─────────────────\n🌐 Facebook:`,
            global.config.ADMINBOT[0],
            event.threadID,
          );
        } else {
